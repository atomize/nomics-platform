...
module.exports = {
  ...
  info: async () => {
    try {
      ...
      au.assertStringProperty(this.info, 'description', {
        required: true
      })
      ...
      if (!process.env.NOMICS_PLATFORM_RELAX) {
        ...
        au.assertStringProperty(this.info, 'location', {
          required: true
        })
        au.assertURLProperty(this.info, 'logo', {
          required: true,
          https: true
        })
      }

      au.assertURLProperty(this.info, 'website', {
        required: true
      })
      au.assertStringProperty(this.info, 'twitter', {
        required: false
      })
      au.assertStringProperty(this.info, 'version');
      if (au.assertProperty(this.info, 'capability', {
          required: false
        })) {
        au.assertPropertyType(this.info, 'capability', 'object')
        const capability = this.info.capability
        au.assertBooleanProperty(capability, 'candles', {
          required: false
        })
        au.assertBooleanProperty(capability, 'markets', {
          required: false
        })
        au.assertBooleanProperty(capability, 'orders', {
          required: false
        })
        au.assertBooleanProperty(capability, 'ordersSnapshot', {
          required: false
        })
        au.assertBooleanProperty(capability, 'ordersSocket', {
          required: false
        })
        au.assertBooleanProperty(capability, 'ticker', {
          required: false
        })
        au.assertBooleanProperty(capability, 'trades', {
          required: false
        })
        au.assertBooleanProperty(capability, 'tradesByTimestamp', {
          required: false
        })
        au.assertBooleanProperty(capability, 'tradesSocket', {
          required: false
        })
      }
  ...
  markets: async () => {
    ...
    try {
      ...
      data.forEach((m) => au.assertStringProperty(m, 'settlement', {
        required: false
      }));
      data.forEach((m) => au.assertStringProperty(m, 'underlying', {
        required: false
      }));
      data.forEach((m) => {
        if (m.type === 'derivative') {
          au.assertPropertyInSet(m, 'subtypes', ['future', 'option', 'perpetual'], {
            required: false
          });
        } else {
          au.assertPropertyInSet(m, 'subtypes', [], {
            required: false
          });
        }
      });
      data.forEach((m) => au.assertURLProperty(m, 'market_url', {
        required: false
      }));
      data.forEach((m) => au.assertStringProperty(m, 'description', {
        required: false
      }));
  ...
  trades: async () => {
    ...
    for (let i = 0; i < markets.length && !success; i++) {
      ...
      try {
        ...
        trades.forEach(t => au.assertPropertyInSet(t, 'type', TRADE_TYPES, {
          required: false
        }))
        trades.forEach((t) => au.assertPropertyInSet(t, 'side', ['buy', 'sell'], {
          required: false
        }));
        trades.forEach((t) => au.assertStringProperty(t, 'order', {
          required: false
        }));
  ...
  ticker: async () => {
    ...
    for (let i = 0; i < markets.length && !success; i++) {
      ...
      try {
        ...
        au.assertNumericStringProperty(ticker, 'close', {
          required: true
        })
        au.assertNumericStringProperty(ticker, 'high', {
          required: false
        })
        au.assertNumericStringProperty(ticker, 'low', {
          required: false
        })
        au.assertProperty(ticker, 'raw', {
          required: true
        })
        au.assertTimestampProperty(ticker, 'timestamp', {
          required: true
        })
        ...
        if (ticker.ask) {
          au.assertNumericStringProperty(ticker, 'ask', {
            required: false
          })
          au.assert(Number(ticker.ask) > 0, 'Expected ask to be greater than zero')
        }
        if (ticker.bid) {
          au.assertNumericStringProperty(ticker, 'bid', {
            required: false
          })
